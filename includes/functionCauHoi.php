...
function searchCauHoitrongDeThi($connect, $searchText, $monHoc, $doKho,$madethi,$ma_nguoi_tao) {
    $sql = "SELECT * FROM cau_hoi WHERE trang_thai=1 AND (noi_dung LIKE '%$searchText%') AND ma_nguoi_Tao=$ma_nguoi_tao AND ma_cau_hoi NOT IN(SELECT ma_cau_hoi FROM chi_tiet_de_thi WHERE ma_de_thi = $madethi ) ";
